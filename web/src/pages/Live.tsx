...
export default function Live() {
  ...
  ;(liveData?.games ?? []).forEach(g => {
    ...
    if (g.status === 'live' || g.status === 'final') liveMap.set(key, g)
  })

  const unified: UnifiedGame[] = (todayData?.games ?? []).map(g => {
    ...
    const liveKey = [g.home_tri, g.away_tri].sort().join('-')
    const live = liveMap.get(liveKey)
    const effectiveLive = live?.status === 'live'
    const effectiveFinal = isLive || isFinal || live?.status === 'final'

    return {
      id: `${g.home_tri}-vs-${g.away_tri}`,
      ...
      home_score: effectiveLive && live ? live.homeScore : g.home_score,
      away_score: effectiveLive && live ? live.awayScore : g.away_score,
      status: effectiveLive ? 'live' : effectiveFinal ? 'final' : 'upcoming',
      period: effectiveLive && live ? live.period : undefined,
      clock:  effectiveLive && live ? live.gameClock : undefined,
      start_time: g.start_time, venue: g.venue,
      sortKey: effectiveLive ? 0 : effectiveFinal ? 2 : 1,
    }
  })
